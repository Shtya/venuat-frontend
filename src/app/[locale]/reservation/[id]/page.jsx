...
const Page = ({ params }) => {
    ...
    const { loadingReservation, loadingPricing, Package , venue, loading, errors, trigger, setValue, submit, watch } = hookConfirmReservation({ id: params.id });
    ...
    return (
        <main className='min-h-[100vh]  container'>
            ...
            {loading ? (
                ...
                <>
                    <div className='max-lg:grid-cols-1 grid grid-cols-2 items-center relative my-[20px] mb-[80px]'>
                        <div className='p-[20px] max-lg:max-w-[500px] w-full max-lg:mx-auto'>
                            <div className='grid grid-cols-2 max-sm:grid-cols-1 gap-[20px]'>
                                <Calendar KEY='check_in' label={t('startReservation')} error={errors?.check_in} setValue={setValue} watch={watch} trigger={trigger} place={t('from')} classname={'w-full '} cnInput='px-[20px]  !border-secondry3 !border-[1px] !h-[45px] rounded-[4px]' />
                                <Calendar KEY='check_out' label={t('endReservation')} error={errors?.check_out} setValue={setValue} watch={watch} trigger={trigger} place={t('to')} classname={'w-full'} cnInput=' px-[20px]  !border-secondry3 !border-[1px] !h-[45px] rounded-[4px]' />
                            </div>
                            ...
                            <Address_vesitor titleAddress={venue?.name?.[locale]} location={`${venue?.property?.city.name} , ${venue?.property?.city?.country.name}`} titleVistor={t('visitors_count')} e={venue} />
            ...
            {!loading && <CountdownTimer Package={Package} />}
            <div className='max-w-[950px] mt-[80px] mx-auto '>
                ...
                <div className='h2 my-[10px] flex items-center justify-between gap-[10px] flex-wrap'> <span> {t('vatValue')} </span> {loadingPricing ? <div className='h-4 bg-gray-200 rounded w-20 animate-pulse'></div> : <span className='font-[700] text-[18px]  text-primary1 ' >15%</span> } </div>
