...
export default function Navbar() {
  ...
  const links = [
    ...
    { cn: 'hidden dashboard-btn !text-white rounded-full px-[20px] py-[5px] font-[600] bg-primary1  ', name: t('dashboard'), value: process.env.NEXT_PUBLIC_LINK_DASHBOARD }
  ];
  ...
  return (
    <nav className={`navbar z-[1000] sticky top-0 ${place ? (isScrolled ? 'bg-black' : 'h-0') : 'bg-white shadow-sm'}`}>
      {/* Mobile Header */}
            <MobileMenu path={path} show={show} handleToggle={handleToggle} place={place} links={links} user={user} t={t} MyAccount={'/my-account'} SignIn={'/sign-in'} />
      ...
      {/* <div className={` lg:hidden fixed z-[1000] w-full top-0 flex items-center justify-between px-5 py-4 ${place ? (isScrolled ? 'bg-black' : 'h-20') : 'bg-white shadow-md'}`}>
        <div className="flex items-center gap-3">
          <Image onClick={handleToggle} className="cursor-pointer hover:opacity-80 transition-opacity" src={`/assets/${show === 'fixed' ? 'close' : `${place ? 'menu-white' : 'menu'}`}.svg`} alt="menu" width={25} height={25} />
          <DropdownLang color={place ? 'text-white' : 'text-secondary1'} />
        </div>
        <Link href="/">
          <Image src={`/assets/${place ? 'logo-white' : 'logo'}.svg`} alt="logo" width={130} height={60} className="transition-transform hover:scale-105" />
        </Link>
      </div> */}
      ...
      >
        <div className="flex items-center gap-10 max-lg:flex-col max-lg:items-start max-lg:gap-8 max-lg:mt-[-180px]">
          <Link className="max-lg:hidden hover:opacity-90 transition-opacity" href="/">
            <Image src={`/assets/${place ? 'logo-white' : 'logo'}.svg`} alt="logo" width={100} height={40} />
          </Link>
        ...
        <div className="flex items-center gap-6 max-lg:flex-col max-lg:items-start max-lg:gap-5">
          <DropdownLang color={place ? 'text-white' : 'text-secondary1'} classname="max-lg:hidden" />

          {user ? (
            <Link href={MyAccount} className="flex items-center gap-2 cursor-pointer group">
              <div className="w-9 h-9 flex items-center justify-center rounded-full bg-primary3 border border-gray-200 overflow-hidden group-hover:ring-2 group-hover:ring-primary1 transition-all">
                <Image className="w-full h-full object-cover" src={user?.avatar || ImgUserPrimary} alt="user avatar" width={36} height={36} />
              </div>
              <div className="flex flex-col max-lg:gap-1">
                <div className={`text-xs ${place ? 'text-white' : 'text-secondary1'} font-medium opacity-80`}>{t('hello')}</div>
                <div className={`text-sm ${place ? 'text-white' : 'text-secondary1'} font-semibold capitalize  transition-colors`}>{user?.full_name?.split(' ')[0]}</div>
              </div>
            </Link>
          ) : (
            <Link href={SignIn} className={`${place ? 'text-white' : 'text-secondary1'} hover:text-primary1 transition-colors px-3 py-1 rounded-lg ${place ? 'hover:bg-white hover:bg-opacity-10' : 'hover:bg-gray-100'}`}>
              {t('login')}
            </Link>
          )}

          <Button href={'/sign-up/provider?step=1'} showIcon={true} name={t('add_your_hall_now')} className="hover:shadow-lg transition-shadow" />
        </div>
      </div>
