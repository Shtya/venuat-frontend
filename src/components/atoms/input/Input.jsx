...
import { IconSAR } from '../SAR'


export default function Input({ unite , IconUnit  , place , dataAos , rounded=true , error , cnInput , classname , label , type , icon , KEY , register , cnLabel}) {
  const t = useTranslations()
	...
	<div data-aos={dataAos} className={`${classname} flex flex-col  gap-[5px]  relative `} >
		...
		<div className={`border-b-[#BCBBBF]  w-full ${rounded ? "!rounded-[8px]" : "!rounded-[0px]"} overflow-hidden  border-b-[1px] relative  h-[45px] overflow-hidden ${cnInput} `} >
			...
			{unite 
				? IconUnit
					? <IconSAR classname={'absolute text-[12px] max-sm:text-[10px]  font-semibold ltr:right-[10px] rtl:left-[10px] top-[50%] translate-y-[-50%] '} />
					:<span className='absolute text-[12px] max-sm:text-[10px]  font-semibold ltr:right-[10px] rtl:left-[10px] top-[50%] translate-y-[-50%] ' > {unite} </span> 
				: null
			}
