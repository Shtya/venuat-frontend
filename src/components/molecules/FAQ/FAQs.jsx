...
export default function FAQs({unite , data, cnTitle, cn }) {
        ...
        <div className={`py-14 min-h-[100px] bg-[#fdfdfd] ${cn} max-w-[900px] w-full`}>
            <div className='min-h-[100px] flex flex-col justify-center'>
                ...
                <div data-aos='fade-up' className=' flex flex-col gap-[20px] '>
                    {Array.from({ length: 5 }).map((faq, index) => (
                        <div key={index} className={`  rounded-[30px] max-sm:rounded-[10px] max-sm:px-[10px] px-[30px] shadow-blur  ${selected === index && 'rounded-[10px] border-gray1 border-[1px] '} duration-300 `}>
                            <button onClick={() => toggleSection(index)} className='w-full  py-4 flex justify-between gap-[5px] items-center'>
                                <div className={`h3 rtl:text-right ltr:text-left font-semibold flex items-center gap-[15px] ${unite && "font-[400] text-secondry3 "} `}>
                                  {unite && <span className='tracking-[-3px] font-[600] h3 text-secondry1 ' > {unite} {index + 1} </span>}
                                  {t.raw('faqs')[index].question}
                                </div>
                            ...
                            <div
                                ...
                                <div className='py-4 px-3 text-gray-600'>{t.raw('faqs')[index].answer}</div>
                            </div>
                        </div>
                    ))}
                </div>
