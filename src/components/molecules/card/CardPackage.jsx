...
export default function CardPackage({ loading , data , Package , btnName }) {
    ...
    useEffect(() => {
        ...
        arrow.style.display = Package?.length === 0 ? 'none' : 'flex';
        
    }, [Package , loading]);

    return (
        <Swiper  {...config} className={` ${loading ? "min-h-[200px] " : "" } mySwiper  w-full !bg-white mb-[80px]`}>
            {
            loading
              ...
              : Package?.map((e, i) => ( 
                <SwiperSlide className='!bg-white shadow-none' key={e.id}  >
                    <InnerCard activeAos={false} btnName={btnName} e={e} i={i} data={data} /> 
                </SwiperSlide>
              ))}
        </Swiper>
    );
