...
export default function SelectEqupment({previousStep , loading, fields , append , watch , remove , register ,trigger, errors,  submit, setValue }) {
    ...
    return (
        <div data-aos="fade-up" className='flex flex-col gap-[20px]'>
			...
			<div className='flex flex-col gap-[30px] ' >
				{
					...
					<div key={i}  className='grid max-md:grid-cols-1 grid-cols-[60px,1fr,1fr,1fr]  items-center gap-x-[10px] gap-y-[20px]  ' > 
                        ...
                        <Select  sendId={true} KEY={`equipment.${i}.equipment_id`} error={errors?.equipment?.[i]?.equipment_id} setValue={setValue} watch={watch} trigger={trigger} data={equipments} label={t('equipment_name')} place={t('enter_equipment_name')} cnSelect=' px-[10px] rounded-[8px] !border-[#646369] !border-[1px] ' />
                        ...
                        <Input unite={t("sar")} register={register(`equipment.${i}.price`)} error={errors?.equipment?.[i]?.price} label={t("price2")}     KEY={"price"}                 type={"number"}   place={t("enter_price")}   cnInput="!border-secondry3 !border-[1px] " />
            ...
            <Popup title={t('addNewEquipment')} isOpen={isOpenPopup} onClose={() => setisOpenPopup(false)} content={ <AddNewEquipmentPopup onClose={() => setisOpenPopup(false)} /> } />
